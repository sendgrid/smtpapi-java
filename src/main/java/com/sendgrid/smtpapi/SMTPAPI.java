...
public class SMTPAPI {
  ...
  public SMTPAPI addSubstitution(String key, String val) throws JSONException {
    if (this.header.isNull("sub")) this.header.put("sub", new JSONObject());
    JSONObject subs = this.header.getJSONObject("sub");
    if (!subs.has(key)) subs.put(key, new JSONArray());
    subs.accumulate(key, val);
    return this;
  }

  public SMTPAPI addSubstitutions(String key, String[] val) throws JSONException {
    for (int i = 0; i < val.length; i++) addSubstitution(key, val[i]);
    return this;
  }
  ...
  public SMTPAPI addUniqueArg(String key, String val) throws JSONException {
    if (this.header.isNull("unique_args")) this.header.put("unique_args", new JSONObject());
  ...
  public SMTPAPI addCategory(String val) throws JSONException {
    if (!this.header.has("category")) this.header.put("category", new JSONArray());
    this.header.accumulate("category", val);
    return this;
  }

  public SMTPAPI addCategories(String[] vals) throws JSONException {
    for (int i = 0; i < vals.length; i++) addCategory(vals[i]);
    return this;
  }
  ...
  public SMTPAPI addSection(String key, String val) throws JSONException {
    if (this.header.isNull("section")) this.header.put("section", new JSONObject());
  ...
  public SMTPAPI addFilter(String filter, String setting, String val) throws JSONException {
    if (this.header.isNull("filters")) this.header.put("filters", new JSONObject());
  ...
  private int[] toCodePointArray(String input) {
    ...
    for (int i = 0, j = 0; i < len; i = input.offsetByCodePoints(i, 1)) codePointArray[j++] = input.codePointAt(i);
  ...
  private String escapeUnicode(String input) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      } else if (codePointArray[i] > 127) sb.append(String.format("\\u%04x",codePointArray[i]));
      else sb.append(String.format("%c", codePointArray[i]));
    }
    return sb.toString();
  }
